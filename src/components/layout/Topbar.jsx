  ...
  return (
    <header className="bg-white/40 backdrop-blur-xl border border-white/60 rounded-[20px] px-8 py-4 flex justify-between items-center shadow-lg relative z-[999]">
      <div className="flex items-center gap-4">
        {/* MODIFIKASI: Tambahkan stopPropagation agar klik burger tidak dianggap klik ke tempat lain */}
        <button 
          onClick={(e) => {
            e.stopPropagation(); 
            toggleSidebar();
      ...
      >
        <div className="text-right hidden sm:block">
          <p className="font-bold text-sm leading-none mb-1">Vicky</p>
          <p className="text-orange-500 text-xs font-semibold leading-none">Host</p>
        </div>
        ...
        {isDropdownOpen && (
          <div className="absolute top-[125%] right-0 w-44 bg-white/90 backdrop-blur-md border border-white/60 rounded-2xl shadow-xl py-2 animate-slide-up z-[1001]">
            ...
            {/* Logout diarahkan ke root / */}
            <a href="/" className="w-full px-5 py-3 text-left text-sm flex items-center gap-2 text-red-500 hover:bg-red-50 transition-colors font-medium">
